...
func _refresh() -> void:
	visible = (
		_controller.has_cell_context()
		or _controller.get_mode() != BuildablePlacementController.Mode.IDLE
	)
	if not visible:
		return
	...
	var selected_snapshot: BuildableSnapshot = _grid.get_snapshot(selected_id)

	_medical_button.visible = (
		has_empty_cell
		and _grid.balance.is_medical_cell(selected_cell)
	)
	_turret_button.visible = (
		has_empty_cell
		and _grid.balance.is_turret_cell(selected_cell)
	)
	_move_button.visible = (
		has_object
		and selected_snapshot != null
		and selected_snapshot.type_id != BuildableType.Id.MEDICAL_STATION
	)
	...
	_medical_button.disabled = (
		not commands_enabled
		or not _controller.can_place_type_in_selected_cell(
			BuildableType.Id.MEDICAL_STATION
		)
	)
	_turret_button.disabled = (
		not commands_enabled
		or not _controller.can_place_type_in_selected_cell(BuildableType.Id.TURRET)
	)
