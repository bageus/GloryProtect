...
func get_shield_recharge_multiplier(orb_id: int) -> float:
	...
	var distance: float = absf(_platform.position.x - _orbs.get_world_x(orb_id))
...
func get_last_anchor_point(side: int) -> Vector2:
	return _last_anchor_points.get(side, Vector2(INF, INF))
...
func _damage_near(
	...
	for enemy: BoardingEnemy in _enemies.get_all_enemies():
		...
		if target_kind == TARGET_GROUND and not enemy.is_counted_as_ground():
			continue
		if target_kind == TARGET_NON_GROUND and enemy.is_counted_as_ground():
			continue
