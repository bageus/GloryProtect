...
func _physics_process(delta: float) -> void:
	if not _game_flow.is_world_simulation_active():
		return
	if not upgrades.long_flight_restore_enabled:
		_flight_seconds = 0.0
	elif not _contact.is_contact_active():
...
func apply_upgrade_effect(effect: UpgradeEffectDefinition) -> bool:
	...
	if not applied:
		return false
	if effect.target_id == AnchorlessControlUpgradeRuntime.SPEED_LONG_FLIGHT_RESTORE:
		_flight_seconds = 0.0
	_sync_motion_modifiers()
...
func _damage_matching_targets(
	...
	for enemy: BoardingEnemy in _enemies.get_all_enemies():
		if enemy == null or not enemy.health.is_alive():
			continue
		if not _matches_target_kind(enemy, target_kind):
			continue
		enemy.health.apply_damage(amount, source_id, self)
		damaged_count += 1
...
func _matches_target_kind(enemy: BoardingEnemy, target_kind: int) -> bool:
	match target_kind:
		TARGET_GROUND:
			return enemy.is_counted_as_ground()
		TARGET_PLATFORM_OR_AIR:
			return _is_platform_or_air_target(enemy)
	return false
