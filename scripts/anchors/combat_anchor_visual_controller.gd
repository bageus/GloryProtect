...
func configure_combat(
	...
	if (
		...
		and not _combat_anchors.upgrades_changed.is_connected(_on_upgrades_changed)
	):
		_combat_anchors.upgrades_changed.connect(_on_upgrades_changed)


func is_electric_visual_active(anchor_id: int) -> bool:
	if _combat_anchors == null or anchor_id < 0:
		return false
	if not (
		...
		return false
	var state: int = _combat_anchors._anchors.get_anchor_state(anchor_id)
	return state in [
...
func _draw_attached_anchor(
	...
	if not is_electric_visual_active(anchor.anchor_id):
		return
	_draw_electric_arcs(start, _get_clamp_connection_point(ground), anchor.anchor_id)
...
func _draw_electric_arcs(
	...
	var phase: float = _warning_elapsed * electric_arc_speed + float(anchor_id) * 1.91
	...
	for index: int in range(point_count + 1):
		...
		arc.append(position)
	draw_polyline(arc, Color(0.3, 0.82, 1.0, 0.55), 5.0, true)
	draw_polyline(arc, Color(0.88, 0.98, 1.0, 1.0), 2.0, true)
	...
	for branch_index: int in range(branch_count):
		...
		draw_line(origin, branch_end, Color(0.75, 0.96, 1.0, 0.9), 1.5, true)
