...
func _run() -> void:
	...
	var rewards: BoardingRewardController = game.get_node("World/BoardingRewardController")
	...
	assert(rewards.get_coin_counter_text() == "Монеты: 3")
	assert(rewards.is_coin_gain_visible())
	assert(rewards.get_coin_gain_text() == "+3")
...
func _disable_spawners(game: Node) -> void:
	for path: NodePath in [NodePath("World/BoardingSpawnDirector"), NodePath("World/StrategicWaveDirector"), NodePath("World/StrategicGroupMutationController")]:
