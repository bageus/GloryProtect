...
func _test_piercing_and_explosion() -> void:
	...
	var primary := await _make_enemy(registry, Vector2(50, 0), 5, BoardingEnemyController.State.ON_PLATFORM)
	var behind_one := await _make_enemy(registry, Vector2(80, 0), 5, BoardingEnemyController.State.ON_PLATFORM)
	var behind_two := await _make_enemy(registry, Vector2(110, 0), 5, BoardingEnemyController.State.ON_PLATFORM)
	var outside_lane := await _make_enemy(registry, Vector2(90, 80), 5, BoardingEnemyController.State.ON_PLATFORM)
	var invalid_ground := await _make_enemy(registry, Vector2(60, 40), 5, BoardingEnemyController.State.WAITING_WITHOUT_PATH)
	var outside_range := await _make_enemy(registry, Vector2(250, 0), 5, BoardingEnemyController.State.ON_PLATFORM)
	var upgrades := ShooterUpgradeRuntime.new()
	assert(upgrades.apply_flag(&"shooter_specialization_sniper"))
	assert(upgrades.apply_flag(&"shooter_sniper_multi_pierce"))
	assert(upgrades.apply_flag(&"shooter_sniper_explosive_fifth"))
	var resolver := ShooterCombatResolver.new()
	resolver.configure(BALANCE)
	resolver.resolve_bolt_hit(shooter, primary, registry, POLICY, upgrades, 1, 5, 150.0)
...
func _test_angled_piercing_follows_bolt_vector() -> void:
	...
	var shooter := Defender.new()
	shooter.position = Vector2.ZERO
	var primary := await _make_enemy(registry, Vector2(50, 50), 3, BoardingEnemyController.State.ON_PLATFORM)
	var inline := await _make_enemy(registry, Vector2(80, 80), 3, BoardingEnemyController.State.ON_PLATFORM)
	var horizontal_only := await _make_enemy(registry, Vector2(90, 50), 3, BoardingEnemyController.State.ON_PLATFORM)
	var upgrades := ShooterUpgradeRuntime.new()
	assert(upgrades.apply_flag(&"shooter_piercing_bolt"))
	var resolver := ShooterCombatResolver.new()
	resolver.configure(BALANCE)
	resolver.resolve_bolt_hit(shooter, primary, registry, POLICY, upgrades, 1, 1, 150.0)
...
func _test_air_mark_selects_strongest_target() -> void:
	var registry := _make_registry()
	var shooter := Defender.new()
	var primary := await _make_air_enemy(registry, Vector2(20, 0), 2)
	var strongest := await _make_air_enemy(registry, Vector2(40, 0), 6)
	var weaker := await _make_air_enemy(registry, Vector2(60, 0), 3)
	var upgrades := ShooterUpgradeRuntime.new()
	assert(upgrades.apply_flag(&"shooter_specialization_air_hunter"))
	assert(upgrades.apply_flag(&"shooter_air_mark_fifth"))
	var resolver := ShooterCombatResolver.new()
	resolver.configure(BALANCE)
	resolver.resolve_bolt_hit(shooter, primary, registry, POLICY, upgrades, 1, 5, 200.0)
...
func _test_anchor_knockdown_uses_common_death_flow() -> void:
	var registry := _make_registry()
	var climbing := await _make_enemy(
		registry,
		Vector2(30, 0),
		5,
	...
	registry.enemy_removed.connect(func(_enemy_id: int, reason: StringName) -> void:
		removed_reasons.append(reason)
	)
	var upgrades := ShooterUpgradeRuntime.new()
	assert(upgrades.apply_flag(&"shooter_specialization_anchor_hunter"))
	assert(upgrades.apply_flag(&"shooter_anchor_knockdown_fifth"))
	var resolver := ShooterCombatResolver.new()
	resolver.configure(BALANCE)
	assert(resolver.resolve_volley_finished(climbing, upgrades, 5))
	assert(removed_reasons == [&"shooter_anchor_knockdown"])
	assert(registry.get_enemy(climbing.enemy_id) == null)
	registry.queue_free()


func _make_registry() -> BoardingEnemyRegistry:
